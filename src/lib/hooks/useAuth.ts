...
const useAuth = (
  email: string,
  password: string
): Promise<ILoggedInUser | null> => {
  if (isNullOrEmpty(email)) return Promise.reject("Email is invalid");

  if (isNullOrEmpty(password)) return Promise.reject("Email is invalid");
  try {
