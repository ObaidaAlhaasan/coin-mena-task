...
import {validateEmail, validatePassword, validateText} from "../../../utils/string-utils";

export type validationInput = { value: string, isValid: boolean | null, validationMsg: string | null };
...
export const InputWithValidation: FC<IInputWithValidation> = (props) => {
  ...
  const onValidate = (value: string) => {
    if (props.type === "text" && !validateText(value))
      return setInput({
        ...input,
        isValid: false,
    ...
    if (props.type === "password" && !validatePassword(value))
      return setInput({
        ...input, isValid: false,
        validationMsg: 'Password field Is Invalid should contain one special character, one number'
      });
  ...
  return (
    <>
      ...
      <input id={props.id}
             type={props.type}
             className={`form-control ${props.input.value && (props.input.isValid ? 'is-valid' : 'is-invalid')} ${props.className}`}
             value={input.value}
